...
# images
PIL_sidebar = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "side-bar.png")
)
PIL_logo_incl_text = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "square_logo_incl_text.png")
)
PIL_checkmark = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "checkmark.png")
)
PIL_dir_image = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "image-gallery.png")
)
PIL_mdl_image = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "tech.png")
)
PIL_spp_image = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "paw.png")
)
PIL_run_image = PIL.Image.open(
    os.path.join(AddaxAI_files, "AddaxAI", "imgs", "shuttle.png")
)
...
of_txt = ["of", "de"]
suffixes_for_sim_none = [
    " - just show me where the animals are",
...
def postprocess(
    ...
    if plt and not exp:
        ...
        if not (
            os.path.isfile(os.path.join(dst_dir, "results_detections.csv"))
            and os.path.isfile(os.path.join(dst_dir, "results_files.csv"))
        ):
            exp = True
    ...
    if data_type == "img" and get_hitl_var_in_json(recognition_file) == "in-progress":
        if not mb.askyesno(
            ...
            f"Your verification session is not yet done. You can finish the session "
    ...
    if data_type == "vid":
        if vis or crp or plt:
            check_json_presence_and_warn_user(
                ["visualize, crop, or plot", "visualizar, recortar o trazar"][lang_idx],
                [
                    "visualizing, cropping, or plotting",
                    "visualizando, recortando o trazando",
                ][lang_idx],
                [
                    "visualization, cropping, and plotting",
                    "visualización, recorte y trazado",
                ][lang_idx],
            )
            vis, crp, plt = [False] * 3
    ...
    if (
        exp and exp_format == dpd_options_exp_format[lang_idx][0]
    ):  # if exp_format is the first option in the dropdown menu -> XLSX
        n_rows_files = 1
        n_rows_detections = 1
        ...
        if n_rows_detections > 1048576 or n_rows_files > 1048576:
            mb.showerror(
                ["To many rows", "Demasiadas filas"][lang_idx],
                [
    ...
    for image in data["images"]:
        ...
        if "failure" in image:
            ...
            with open(postprocessing_error_log, "a+") as f:
                f.write(
                    f"File '{image['file']}' was skipped by post processing features because '{image['failure']}'\n"
                )
            f.close()

            # calculate stats
            elapsed_time_sep = str(
                datetime.timedelta(seconds=round(time.time() - start_time))
            )
            time_left_sep = str(
                datetime.timedelta(
                    seconds=round(
                        ((time.time() - start_time) * n_images / nloop)
        ...
        if vis or crp or exp:
            if data_type == "img":
                ...
                if im_to_vis is None:
                    with open(postprocessing_error_log, "a+") as f:
                        f.write(
                            f"File '{image['file']}' was skipped by post processing features. This might be due to the file being moved or deleted after analysis, or because of a special character in the file path.\n"
                        )
                    f.close()
                    elapsed_time_sep = str(
                        datetime.timedelta(seconds=round(time.time() - start_time))
            ...
            if exp:
                ...
                try:
                    ...
                    if "Latitude" in gpsinfo and "Longitude" in gpsinfo:
                        gpsinfo["GPSLink"] = (
                            f"https://maps.google.com/?q={gpsinfo['Latitude']},{gpsinfo['Longitude']}"
                        )
                except:
        ...
        if exp:
            ...
            row.to_csv(
                csv_for_files, encoding="utf-8", mode="a", index=False, header=False
            )
            ...
            if var_vis_bbox.get():
                for bbox in bbox_info:
                    if manually_checked:
                        ...
                        conf_label = (
                            round(bbox[1], 2) if round(bbox[1], 2) != 1.0 else 0.99
                        )
                    ...
                    bb.add(
                        ...
                        *bbox[3:7],
                        vis_label,
                        color,
                        size=dpd_options_vis_size[lang_idx].index(var_vis_size.get()),
                    )  # convert string to index, e.g. "small" -> 0
        ...
        if crp and len(bbox_info) > 0:
            ...
            for bbox in bbox_info:
                ...
                im_path = os.path.join(
                    dst_dir,
                    filename + "_crop" + str(counter) + "_" + bbox[0] + file_extension,
                )
    ...
    if exp:
        ...
        det_info = pd.DataFrame(
            pd.read_csv(csv_for_detections, dtype=dtypes, low_memory=False)
        )
        summary = pd.DataFrame(
            det_info.groupby(["label", "data_type"])
            .size()
            .sort_values(ascending=False)
            .reset_index(name="n_detections")
        )
        summary.to_csv(
            csv_for_summary, encoding="utf-8", mode="w", index=False, header=True
        )
    ...
    if (
        ...
        for result_type in ["detections", "files", "summary"]:
            ...
            if os.path.isfile(xlsx_path):
                ...
                df_csv = pd.read_csv(
                    os.path.join(dst_dir, f"results_{result_type}.csv"),
                    dtype=dtypes,
                    low_memory=False,
                )
    ...
    if (
        exp and exp_format == dpd_options_exp_format[lang_idx][3]
    ):  # if exp_format is the third option in the dropdown menu -> TSV
        ...
        if os.path.isfile(tsv_path):  # Append if TSV exists
            with (
                open(csv_path, "r", newline="") as csv_file,
                ...
                idx_date, idx_lat, idx_lon = map(
                    csv_header.index, ["DateTimeOriginal", "Latitude", "Longitude"]
                )
            ...
            with (
                ...
                tsv_writer.writerow(
                    ["ID", "Date", "Lat", "Long", "Method"] + csv_header
                )
    ...
    if exp and exp_format == dpd_options_exp_format[lang_idx][2]:  # COCO
        ...
        detections_df = pd.read_csv(
            os.path.join(dst_dir, f"results_detections.csv"),
            dtype=dtypes,
            low_memory=False,
        )
        files_df = pd.read_csv(
            os.path.join(dst_dir, f"results_files.csv"), dtype=dtypes, low_memory=False
        )

        # convert csv to coco format
        csv_to_coco(
            detections_df=detections_df, files_df=files_df, output_path=coco_path
        )
    ...
    if plt:
        ...
        if (
            (exp and exp_format == dpd_options_exp_format[lang_idx][0])
            or (exp and exp_format == dpd_options_exp_format[lang_idx][2])
            or (exp and exp_format == dpd_options_exp_format[lang_idx][3])
            or remove_csv
        ):
            for result_type in ["detections", "files", "summary"]:
                csv_path = os.path.join(dst_dir, f"results_{result_type}.csv")
...
def csv_to_coco(detections_df, files_df, output_path):
    ...
    coco = {
        ...
        "annotations": [],
        "categories": [],
        "licenses": [{"id": 1, "name": "Unknown", "url": "NA"}],
        "info": {
    ...
    for label in detections_df["label"].unique():
        if label not in category_mapping:
            ...
            coco["categories"].append({"id": current_category_id, "name": label})
            current_category_id += 1
    ...
    for _, file_info in files_df.iterrows():
        ...
        if type(file_info["DateTimeOriginal"]) == float:  # means NA value
            ...
            date_captured = datetime.datetime.strptime(
                file_info["DateTimeOriginal"], "%d/%m/%y %H:%M:%S"
...
def start_postprocess():
    ...
    if not img_json and not vid_json:
        mb.showerror(
            error_txt[lang_idx],
            [
                "No model output file present. Make sure you run step 2 before post-processing the files.",
    ...
    if dst_dir in ["", "/", "\\", ".", "~", ":"] or not os.path.isdir(dst_dir):
        mb.showerror(
            ["Destination folder not set", "Carpeta de destino no establecida."][
                lang_idx
            ],
            [
    ...
    if os.path.normpath(dst_dir) == os.path.normpath(src_dir) and vis and not sep:
        if not mb.askyesno(
            [
                ...
                "Las imágenes originales se sobrescribirán.",
            ][lang_idx],
            [
    ...
    if var_cls_model.get() == "Global - SpeciesNet - Google":
        ...
        if vid_present:
            mb.showerror(
                ["SpeciesNet not available", "SpeciesNet no disponible"][lang_idx],
                message=[
                    f"Video support for SpeciesNet will be available in a future AddaxAI release, please uncheck 'process videos'.",
...
def no_user_input(var):
    if (
        var.get() == ""
        or var.get().startswith("E.g.:")
        or var.get().startswith("Ejem.:")
    ):
        return True
    else:
        return False
...
def invalid_value_warning(str, numeric=True):
    string = [
        f"You either entered an invalid value for the {str}, or none at all.",
        f"Ingresó un valor no válido para {str} o ninguno.",
    ][lang_idx]
    if numeric:
        string += [
            " You can only enter numeric characters.",
            " Solo puede ingresar caracteres numéricos.",
        ][lang_idx]
...
def abs_paths_warning():
    ...
    if var_abs_paths.get() and shown_abs_paths_warning:
        mb.showinfo(
            warning_txt[lang_idx],
            [
                "It is not recommended to use absolute paths in the output file. Third party software (such "
...
def toggle_image_size_for_deploy():
    if var_use_custom_img_size_for_deploy.get():
        lbl_image_size_for_deploy.grid(
            row=row_image_size_for_deploy, sticky="nesw", pady=2
        )
        ent_image_size_for_deploy.grid(
            row=row_image_size_for_deploy, column=1, sticky="nesw", padx=5
        )
    else:
...
def on_chb_smooth_cls_animal_change():
    ...
    if var_smooth_cls_animal.get():
        mb.showinfo(
            information_txt[lang_idx],
            [
                "This feature averages confidence scores to avoid noise. Note that it assumes a single species per "
                "sequence or video and should therefore only be used if multi-species sequences are rare. It does not"
                " affect detections of vehicles or people alongside animals.",
                "Esta función promedia las puntuaciones "
...
def complete_frame(frame):
    ...
    if trd_step or fst_step:
        ...
        if trd_step:
            btn_hitl_main.configure(
                text=["New session?", "¿Nueva sesión?"][lang_idx], state=NORMAL
            )
            btn_hitl_main.lift()
        if fst_step:
            btn_choose_folder.configure(
                text=f"{change_folder_txt[lang_idx]}?", state=NORMAL
            )
        ...
        # add buttons
        btn_view_results = Button(
            master=frame,
            text=view_results_txt[lang_idx],
            width=1,
            command=lambda: view_results(frame),
        )
        btn_view_results.grid(row=0, column=1, sticky="nesw", padx=5)
        btn_uncomplete = Button(
            master=frame,
            text=again_txt[lang_idx],
            width=1,
            command=lambda: enable_frame(frame),
        )
...
def uncomplete_frame(frame):
    ...
    for child in children:
        if child.winfo_class() == "Button" or child.winfo_class() == "Label":
            if (
                child.cget("text") == again_txt[lang_idx]
                or child.cget("text") == view_results_txt[lang_idx]
                or child.cget("image") != ""
            ):
                child.grid_remove()
...
def resize_canvas_to_content():
    ...
    new_advanc_bg_image = customtkinter.CTkImage(
        PIL_sidebar, size=(ADV_WINDOW_WIDTH, bg_height)
    )
...
def switch_mode():
    ...
    # save
    write_global_vars({"advanced_mode": not advanced_mode})
